...
기본적으로 Starship은 경고 및 오류를 `~/.cache/starship/session_${STARSHIP_SESSION_KEY}.log`라는 파일에 기록합니다. 여기서 세션 키는 터미널 인스턴스에 해당합니다.
하지만 이는 `STARSHIP_CACHE` 환경 변수를 사용하여 변경할 수 있습니다.
...
줄 바꿈을 사용할 때는 여러 줄로 된 선언을 사용할 수 있습니다.
예를 들어, `$` 기호를 새 줄에 출력하려면 `format`에 다음 값들을 사용하면 동일한 효과를 얻을 수 있습니다:
...
포맷 문자열은 모듈이 모든 변수를 출력하는 데 사용하는 형식입니다.
대부분의 모듈에는 모듈의 표시 형식을 구성하는 `format`이라는 항목이 있습니다.
형식 문자열에서 텍스트, 변수 및 텍스트 그룹을 사용할 수 있습니다.
...
| `format`            | `'via [$symbol($version )]($style)'` | The format for the module.                                                                    |
| `version_format`    | `'v${raw}'`                          | The version format. Available vars are `raw`, `major`, `minor`, & `patch` |
| `기호`                | `'🅼 '`                              | A format string representing the symbol of Maven.                                             |
| `detect_extensions` | `[]`                                 | Which extensions should trigger this module.                                                  |
...
> 그래도 활성화하려면, [아래에 있는 예시를 따라 하세요](#with-pulumi-version).
...
> 그래도 활성화하려면, [아래에 있는 예시를 따라 하세요](#with-terraform-version).
