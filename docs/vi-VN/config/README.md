...
Định dạng các chuỗi là định dạng một module với việc in ra tất cả các biến của nó.
Most modules have an entry called `format` that configures the display format of the module.
Bạn có thể sử dụng các văn bản, các biến và các nhóm văn bản trong một định dạng chuỗi.
...
Bạn có thể thêm các văn bản, các biến, hoặc thậm chí các nhóm văn bản lồng nhau vào trong nó.
...
Đa số các module trong starship cho phép bạn cấu hình kiểu hiển thị của chúng. This is done with an entry (usually called `style`) which is a string specifying the configuration. Đây là vài ví dụ của các chuỗi kiểu cũng với những gì chúng làm. For details on the full syntax, consult the [advanced config guide](../advanced-config/).
...
The default `format` is used to define the format of the prompt, if empty or no `format` is provided. Mặc định như sau:
...
Module chỉ được nhìn thấy khi pin của thiết bị dưới 10%.
...
If no `display` is provided. Mặc định như sau:
...
Mặc định, nó chỉ thay đổi màu. If you also want to change its shape take a
...
The `cmake` module shows the currently installed version of [CMake](https://cmake.org/). Mặc định
module sẽ được kích hoạt nếu thoả mãn bất kì điều kiện nào dưới đây:
...
three parent folders. Đường dẫn của bạn cũng sẽ được cắt ngắn tới đường dẫn gốc của git repo hiện tại của bạn.
...
<details><summary>Mô đun này có một vài tùy chọn nâng cao để điều khiển cách thư mục được hiển thị.</summary>
...
Mô đun này sử dụng cơ chế của bản thân để phát hiện phiên bản của chính nó. Typically it is twice as fast
...
Mô đun sẽ được hiển thị chỉ khi bất kì điều kiện nào sau đây thỏa mãn:
...
| `biến`     |                                       | Biến môi trường được hiển thị.                                               |
| `ví dụ`    |                                       | Giá trị mặc định được hiển thị khi biến được chọn không được định nghĩa.     |
...
| active  | `ví dụ`       | Tên cấu hình có hiệu lực viết trong `~/.config/gcloud/active_config` |
| symbol  |               | Giá trị ghi đè tuỳ chọn `symbol`                                     |
| style\* |               | Giá trị ghi đè của `style`                                           |
...
_BISECTING_, etc. Nếu có thông tin tiến trình (ví dụ, REBASING 3/10), thông tin đó cũng sẽ được hiển thị.
...
Mô đun sẽ được hiển thị chỉ khi có background jobs đang chạy.
...
| level   | `1`     | Giá trị ghi đè tuỳ chọn `symbol` |
...
| project_name | `my-project` | Giá trị ghi đè tuỳ chọn `symbol` |
| symbol                            |              | Giá trị ghi đè của `style`       |
| style                             |              | Giá trị ghi đè của `style`       |
...
| `detect_extensions` | `['p6', 'pm6', 'pod6', 'raku', 'rakumod']`       | Giá trị ghi đè của `style`                                                                                    |
...
| `detect_extensions` | `['rs']`                             | Giá trị ghi đè tuỳ chọn `symbol`                                                                              |
| `detect_files`      | `['Cargo.toml']`                     | Giá trị ghi đè của `style`                                                                                    |
| `detect_folders`    | `[]`                                 | Những thư mục nào sẽ kích hoạt mô-đun này.                                                    |
| `style`             | `'bold red'`                         | Kiểu cho module.                                                                              |
...
| `format`            | `'via [${symbol}(${version} )]($style)'` | Định dạng cho module.                                                                         |
| `version_format`    | `'v${raw}'`                              | The version format. Available vars are `raw`, `major`, `minor`, & `patch` |
| `detect_extensions` | `['sbt', 'scala']`                       | Giá trị ghi đè của `style`                                                                                    |
...
| `format`        | `'[$symbol$shlvl]($style) '` | Giá trị ghi đè tuỳ chọn `symbol`                                              |
...
| `style`    | `'bold dimmed blue'`             | Giá trị ghi đè của `style`                                       |
...
| `symbol`            | `'🅢  '`                               | Giá trị ghi đè tuỳ chọn `symbol`                                                                                                                                                               |
| `style`             | `'bold blue'`                          | Giá trị ghi đè của `style`                                                                                                                                                                     |
| `format`            | `'via [$symbol$environment]($style) '` | Định dạng cho module.                                                                                                                                                          |
...
| `detect_extensions` | `['tf', 'tfplan', 'tfstate']`                           | Giá trị ghi đè của `style`                                                                                    |
| `detect_files`      | `[]`                                                    | Những tên tệp nào sẽ kích hoạt mô-đun này.                                                    |
...
| workspace | `ví dụ`    | The current Terraform workspace |
| symbol    |            | Giá trị ghi đè của `style`      |
| style\*   |            | Giá trị ghi đè của `style`      |
...
| typst_version | `ví dụ`  | The current Typst version                                            |
| symbol                             |          | Giá trị ghi đè của `style`                                           |
| style\*                            |          | Giá trị ghi đè của `style`                                           |
...
The `xmake` module shows the currently installed version of [XMake](https://xmake.io/). Mặc định
module sẽ được kích hoạt nếu thoả mãn bất kì điều kiện nào dưới đây:
