...
| `palettes`        | `{}`                           | Збірка колірних палітр, які прив’язують [кольори](../advanced-config/#style-strings) до імен, визначених користувачем. Зверніть увагу, що кольорові палітри не можуть посилатися на їх власні визначення кольору. |
...
Якщо ви просто хочете розширити стандартний формат, можете використовувати `$all`;
модулі, які ви явно додали до формату, не будуть подвоєні. Напр.
...
Модуль `aws` показує поточний регіон AWS і профіль, а також час, що залишився до кінця сесії при використанні тимчасових облікових даних.
Для виводу модуль використовує змінні `AWS_REGION`, `AWS_DEFAULT_REGION` і `AWS_PROFILE` та файли `~/.aws/config` і `~/.aws/credentials`, коли треба.

Модуль показуватиме профіль якщо облікові дані знаходяться в `~/.aws/credentials`  або коли `credential_process`, `sso_start_url`, чи `sso_session` визначені в `~/.aws/config`. Крім того, достатньо буде визначити будь-яку змінну `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY` або `AWS_SESSION_TOKEN`.
Якщо для параметра `force_display` встановлено значення `true`, уся доступна інформація буде показуватись, навіть якщо не буде виявлено жодних облікових даних відповідно до наведених вище умов.
...
> [!TIP] Порада
> Типово цей модуль вимкнено.
> Щоб його увімкнути, встановіть значення параметра `disabled` в `false` у вашому файлі налаштувань.
...
Модуль `battery` показує наскільки заряджена батарея пристрою і стан заряджання.
...
### Показ модуля Battery {#battery-display}

Параметр конфігурації `display` використовується для визначення, коли має показуватись індикатор заряду батареї (threshold), який символ використовуватиметься (symbol) і як він виглядає (style).
Якщо параметр `display` не встановлено. Стандартне значення є таким, як зазначено нижче:
...
Модуль `buf` показує поточну встановлену версію [Buf](https://buf.build). Зазвичай, модуль показується якщо поточна тека містить файл конфігурації [`buf.yaml`](https://docs.buf.build/configuration/v1/buf-yaml), [`buf.gen.yaml`](https://docs.buf.build/configuration/v1/buf-gen-yaml) або [`buf.work.yaml`](https://docs.buf.build/configuration/v1/buf-work-yaml).
...
Модуль `bun` показує поточну встановлену версію рушія JavaScript — [bun](https://bun.sh).
...
Модуль `c` показує інформацію про ваш C компілятор. Зазвичай модуль буде показаний, якщо поточна тека містить файли `.c` чи `.h`.
...
Кожна команда представлена списком, який складається з назви виконавчого файлу та аргументів, зазвичай `['mycc', '--version']`. Starship спробує виконати кожну команду, поки не отримає результат в STDOUT.
...
Кожна команда представлена списком, який складається з назви виконавчого файлу та аргументів, зазвичай `['mycpp', '--version']`. Starship спробує виконати кожну команду, поки не отримає результат в STDOUT.
...
> `vimcmd_symbol` підтримується лише у cmd, fish та zsh.
> `vimcmd_replace_one_symbol`, `vimcmd_replace_symbol` та `vimcmd_visual_symbol`
> підтримуються лише у fish через [проблеми з виявленням режиму в zsh на рівні вихідного коду](https://github.com/starship/starship/issues/625#issuecomment-732454148).
...
#### З власним значком помилки {#with-custom-error-shape}
...
Модуль `cobol` показує встановлену версію COBOL.
Типово, модуль показується, якщо виконується будь-яка з наступних умов:
...
Модуль `cmd_duration` показує, скільки часу виконувалась  остання команда.
Модуль буде показаний лише в тому випадку, якщо на виконання команди пішло понад дві секунди або більше, ніж значення змінної `min_time`, якщо воно задане.

> [!WARNING] Попередження
> Не перехоплюйте trap DEBUG у Bash
>
...
Користувачі Bash, яким потрібна функціональність, подібна до preexec, можуть скористатися [фреймворком bash_preexec від rcaloras](https://github.com/rcaloras/bash-preexec).
Просто визначте масиви `preexec_function` і `precmd_functions` перед запуском `eval $(starship init $0)`, а потім продовжуйте як зазвичай.
...
> [!TIP] Порада
> Це не пригнічує власний модифікатор підказки conda, ви можете виконати `conda config --set changeps1 False`.
> Якщо ви використовуєте [pixi](https://pixi.sh), ви можете відключити модифікатор запиту pixi, використовуючи `pixi config set shell.change-ps1 false`.

### Параметри

| Параметр            | Стандартно                             | Опис                                                                                                                                                                                                                                                                |
| ------------------- | -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `truncation_length` | `1`                                    | Кількість тек, на які повинен бути скорочений шлях середовища, якщо середовище було створено за допомогою `conda create -p [path]`. `0` — означає без скорочення. Також дивіться модуль [`directory `](#directory). |
...
Модуль `crystal` показує поточну встановлену версію [Crystal](https://crystal-lang.org/).
Типово, модуль показується, якщо виконується будь-яка з наступних умов:
...
Модуль `daml` показує поточну версію SDK  [Daml](https://www.digitalasset.com/developers) коли ви перебуваєте в кореневій теці проєкту Daml. `sdk-version` у файлі `daml.yaml` буде використовуватись, якщо значення не буде перевизначене змінною середовища `DAML_SDK_VERSION`.
Типово, модуль показується, якщо виконується будь-яка з наступних умов:
...
Модуль `dart` показує поточну встановлену версію [Dart](https://dart.dev/).
Типово, модуль показується, якщо виконується будь-яка з наступних умов:
...
Модуль `deno` показує поточну встановлену версію [Deno](https://deno.land/).
Типово, модуль показується, якщо виконується будь-яка з наступних умов:
...
Використовуючи `regex = true`, ви можете використовувати [регулярні вирази Rust](https://docs.rs/regex/latest/regex/#syntax) у `from`.
...
Модуль `elixir` показує поточну встановлену версію [Elixir](https://elixir-lang.org/) та [Erlang/OTP](https://erlang.org/doc/).
Типово, модуль показується, якщо виконується будь-яка з наступних умов:
...
Модуль `elm` показує поточну встановлену версію [Elm](https://elm-lang.org/).
Типово, модуль показується, якщо виконується будь-яка з наступних умов:
...
Модуль `env_var` показує поточне значення обраних змінних середовища.
...
> [!TIP] Порада
> Порядок показу модулів env_var можна встановити індивідуально, включивши `${env_var.foo}` у `format` верхнього рівня (оскільки він містить крапку, потрібно використовувати `${...}`).
> Типово, модуль `env_var` покаже усі модулі env_var, в тому порядку, в якому вони були визначені.

> [!TIP] Порада
> Кілька змінних середовища можна відобразити за допомогою `.`. (див. приклад) Якщо параметр конфігурації `variable` не встановлено, модуль показуватиме значення змінної під назвою тексту після символу `'.'`.
>
...
Модуль `erlang ` показує поточну встановлену версію [Erlang/OTP](https://erlang.org/doc/).
Типово, модуль показується, якщо виконується будь-яка з наступних умов:
...
Модуль `fennel ` показує поточну встановлену версію Fennel](https://fennel-lang.org).
Типово, модуль показується, якщо виконується будь-яка з наступних умов:
...
Кожна команда представлена списком, який складається з назви виконавчого файлу та аргументів, зазвичай `['myfortran', '--version']`. Starship спробує виконати кожну команду, поки не отримає результат в STDOUT.
...
Модуль `gcloud` показує поточну конфігурацію для [`gcloud`](https://cloud.google.com/sdk/gcloud) CLI.
Він базується на файлі `~/.config/gcloud/active_config` та на `~/.config/gcloud/configurations/config_{CONFIG NAME}` і на змінній середовища  `CLOUDSDK_CONFIG`.
Змінні середовища `CLOUDSDK_CORE_PROJECT` та `CLOUDSDK_COMPUTE_REGION`, якщо вони встановлені, замінюють значення `project` та `region` з активної конфігурації, повторюючи поведінку самого `gcloud`.
...
> [!TIP] Порада
> Модуль Git Status працює дуже повільно у теках Windows (наприклад, у теках `/mnt/c/`) у середовищі WSL.
> Ви можете вимкнути модуль або використати `windows_starship` для використання Windows-версії Starship для тримання `git_status` для цих шляхів.
...
Модуль `gleam ` показує поточну встановлену версію [Gleam](https://gleam.run/).
Типово, модуль показується, якщо виконується будь-яка з наступних умов:
...
Модуль `golang ` показує поточну встановлену версію [Go](https://golang.org/).
Типово, модуль показується, якщо виконується будь-яка з наступних умов:
...
Модуль `guix_shell` показує середовище [guix-shell](https://guix.gnu.org/manual/devel/en/html_node/Invoking-guix-shell.html).
...
Модуль `haxe ` показує поточну встановлену версію [Haxe](https://haxe.org/).
Типово, модуль показується, якщо виконується будь-яка з наступних умов:

- Поточна тека містить файли `project.xml`, `Project.xml`, `application.xml`, `haxelib.json`, `hxformat.json` або `.haxerc`
...
Модуль `helm` показує поточну встановлену версію [Helm](https://helm.sh/).
Типово, модуль показується, якщо виконується будь-яка з наступних умов:
...
| `trim_at`         | `'.'`                                  | Рядок, у якому назва хоста буде обрізано після першого збігу. `'.'` зупиниться після першої точки. `''` вимкне будь-яке обрізання. |
| `detect_env_vars` | `[]`                                   | Які змінні середовища повинні запускати цей модуль.                                                                                                                |
...
Модуль `java ` показує поточну встановлену версію [Java](https://www.oracle.com/java/).
Типово, модуль показується, якщо виконується будь-яка з наступних умов:
...
Модуль `jobs` показує поточну кількість завдань, що виконуються зараз.
Модуль показуватиметься лише у випадку наявності фонових завдань.
Модуль покаже кількість запущених завдань, якщо є хоча б 2 завдання, або більше за значення в `number_threshold`, за наявності.
Модуль покаже символ, якщо є принаймні одне, чи більше за значення `symbol_threshold`, фонове завдання. Ви можете встановити обидва значення в 0, щоб _завжди_ показувати символ і кількість завдань, навіть якщо виконується 0 завдань.
...
> [!WARNING] Попередження
> Параметр `threshold` є застарілим, але якщо ви хочете використовувати його, модуль покаже кількість запущених завдань, якщо їх більш ніж 1, або більше значення конфігурації `threshold`, якщо воно існує. Якщо `threshold` встановлено у 0, то модуль також показуватиметься, коли немає запущених завдань.
...
Модуль `julia` показує поточну встановлену версію [Julia](https://julialang.org/).
Типово, модуль показується, якщо виконується будь-яка з наступних умов:
...
Модуль `kotlin ` показує поточну встановлену версію [Kotlin](https://kotlinlang.org/).
Типово, модуль показується, якщо виконується будь-яка з наступних умов:
...
Показує поточну назву [контексту Kubernetes](https://kubernetes.io/docs/concepts/configuration/organize-cluster-access-kubeconfig/#context) та, якщо це встановлено, простір імен, користувача й назву кластера із файлу kubeconfig.
Простір імен повинен бути встановлений у файлі kubeconfig файл, це можна зробити через `kubectl config set-context starship-context --namespace astronaut`.
Так само можна встановити користувача та  кластер за допомогою `kubectl config set-context starship-context --user starship-user` та `kubectl config set-context context context context --cluster starship-cluster`, відповідно.
Якщо параметр `$KUBECONFIG` встановлено, то модуль буде використовувати його, якщо ні — `~/.kube/config`.
...
Зверніть увагу, що всі регулярні вирази виглядають як `^<pattern>$` і мають збігатись з усім рядком. Регулярний вираз `*_pattern` може мати групи, які можуть зазначатись у відповідних аліасах як `$name` та `$N` (дивіться приклад нижче та [документацію rust Regex::replace](https://docs.rs/regex/latest/regex/struct.Regex.html#method.replace)).
...
Модуль `lua` показує поточну встановлену версію [Lua](http://www.lua.org/).
Типово, модуль показується, якщо виконується будь-яка з наступних умов:
...
Модуль `netns` показує поточний мережевий простір імен.
Для отримання мережевого простору імен використовується `ip netns identify`, тому буде виявлено лише мережеві простори імен, змонтовані за адресою `/var/run/netns`.
...
Модуль `nim` показує поточну встановлену версію [Nim](https://nim-lang.org/).
Типово, модуль показується, якщо виконується будь-яка з наступних умов:
...
Модуль `nix_shell` показує середовище [nix-shell](https://nixos.org/guides/nix-pills/developing-with-nix-shell.html).
...
Модуль `nodejs` показує поточну встановлену версію [Node.js](https://nodejs.org/).
Типово, модуль показується, якщо виконується будь-яка з наступних умов:
...
Крім того, модуль буде автоматично прихований, якщо в теці є файл `deno.json`, `deno.jsonc`, `deno.lock`, `bunfig.toml`, `bun.lock` або `bun.lockb`, що має пріоритет над зазначеними вище умовами.
...
Модуль `ocaml` показує поточну встановлену версію [OCaml](https://ocaml.org/).
Типово, модуль показується, якщо виконується будь-яка з наступних умов:
...
Модуль `opa` показує поточну встановлену версію OPA.
Стандартно модуль буде показаний, якщо поточна тека містить файли `.rego`.
...
Модуль `openstack` показує поточну хмару OpenStack і проєкт. Модуль працює лише тоді, коли встановлено змінну середовища `OS_CLOUD`; у цьому випадку він зчитує файл `clouds.yaml` з будь-якого з [стандартних місць розташування](https://docs.openstack.org/python-openstackclient/latest/configuration/index.html#configuration-files),
...
Модуль `os` показує поточну операційну систему.
Інформація про ОС визначається за допомогою бібліотеки [os_info](https://lib.rs/crates/os_info).

> [!WARNING] Попередження
> Crate [os_info](https://lib.rs/crates/os_info), що використовується цим модулем, може бути не точним в деяких системах.
...
`symbols` дозволяє визначити довільні символи для кожного типу операційної системи.
...
Якщо ви хочете, щоб була додана певна операційна система, створіть [запит на додавання функції](https://github.com/starship/starship/issues/new/choose).
...
Модуль `package` показується, коли поточна тека є сховищем для пакунка, і показує його поточну версію. Наразі модуль підтримує такі пакунки: `npm`, `nimble`, `cargo`, `poetry`, `python`, `composer`, `gradle`, `julia`, `mix`, `helm`, `shards`, `galaxy`, `daml` та `dart` packages.
...
Модуль `perl` показує поточну встановлену версію [Perl](https://www.perl.org/).
Типово, модуль показується, якщо виконується будь-яка з наступних умов:
...
Модуль `php ` показує поточну встановлену версію [PHP](https://www.php.net/).
Типово, модуль показується, якщо виконується будь-яка з наступних умов:
...
> [!TIP] Порада
> Типово версія Pulumi не показується, оскільки вона завантажується на порядок довше, ніж більшість втулків (~70 мс).
> Якщо ви все ще хочете увімкнути показ версії, [дивіться  приклад нижче](#with-pulumi-version).
...
#### З версією Pulumi {#with-pulumi-version}
...
Модуль `purescript` показує поточну встановлену версію [PureScript](https://www.purescript.org/).
Типово, модуль показується, якщо виконується будь-яка з наступних умов:
...
> [!TIP] Порада
> Змінна `python_binary` приймає значення одне з:
>
> - рядок (напр. `'python3'`),
> - список рядків (наприклад, `['python', 'python3']`)
> - список списків рядків, що представляють команди з необовʼязковими аргументами (наприклад, `[['mise', 'exec', '--', 'python'], ['python3']]`)
>
> Starship буде намагатися виконати кожну налаштовану команду, доки не отримає результат.
> Зверніть увагу, що ви можете змінити лише виконуваний файл, який запускає Starship, щоб отримати потрібну версію Python, а не аргументи, що використовуються.
>
> Стандартні значення та порядок для `python_binary` було вибрано так, щоб спочатку ідентифікувати версію Python у середовищах virtualenv/conda (які наразі все ще додають python, незалежно від того, чи вказує він на `python3` чи на `python2`). Це може мати побічний ефект: якщо у вас все ще встановлено системний Python 2, він може бути обраний перед будь-яким Python 3 (принаймні в дистрибутивах Linux, які завжди містять символічне посилання `/usr/bin/python` на Python 2). Якщо ви більше не працюєте з Python 2, але не можете видалити системний Python 2, змінивши його на `'python3'`, вам потрібно приховати будь-яку версію Python 2, див. приклад нижче.
...
[python]
# Потенційно небезпечно: `uv` може запускати будь-який бінарний файл у теці `.venv/bin/python` без запиту користувача
...
- Поточна тека містить файл з розширенням`.Rsx`.
- Поточна тека містить файл `.Rprofile`
- Поточна тека містить теку `.Rproj.user`
...
Модуль `raku ` показує поточну встановлену версію [Raku](https://www.raku.org/).
Типово, модуль показується, якщо виконується будь-яка з наступних умов:
...
| vm_version | `moar`  | Версія віртуальної машини, на якій побудовано `raku` |
| symbol                          |         | Віддзеркалює значення параметра `style`              |
| style\*                         |         | Віддзеркалює значення параметра `style`              |
...
Модуль `red ` показує поточну встановлену версію [Red](https://www.red-lang.org/).
Модуль показується, якщо виконується будь-яка з наступних умов:

- Поточна тека містить файли з розширеннями `.red` або `.reds`
...
Модуль `ruby ` показує поточну встановлену версію [Ruby](https://www.ruby-lang.org/).
Модуль показується, якщо виконується будь-яка з наступних умов:
...
Модуль `rust ` показує поточну встановлену версію [Rust](https://www.rust-lang.org/).
Модуль показується, якщо виконується будь-яка з наступних умов:
...
Модуль `scala ` показує поточну встановлену версію [Scala](https://www.scala-lang.org/).
Типово, модуль показується, якщо виконується будь-яка з наступних умов:
...
Модуль `shlvl` показує значення поточної змінної середовища [`SHLVL`](https://tldp.org/LDP/abs/html/internalvariables.html#SHLVLREF) ('shell level'), якщо вона має числове значення та дорівнює або перевищує вказаний поріг.
...
Модуль `singularity` показує поточний образ [Singularity](https://sylabs.io/singularity/), якщо система працює в контейнері та встановлено змінну `$SINGULARITY_NAME`.
...
Модуль `spack` показує поточне середовище [Spack](https://spack.readthedocs.io/en/latest/), якщо встановлено змінну `$SPACK_ENV`.

### Змінні

| Змінна              | Приклад                                | Опис                                                                                                                                                                                                    |
| ------------------- | -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `truncation_length` | `1`                                    | Кількість тек, до яких має бути скорочений шлях до середовища оточення. `0` — означає без скорочення. Також дивіться модуль [`directory `](#directory). |
| `symbol`            | `'🅢  '`                               | Віддзеркалює значення параметра `symbol`                                                                                                                                                                |
...
Модуль `status` показує код завершення роботи попередньої команди.
Якщо $success_symbol пустий (типово), модуль буде показаний тільки якщо код виходу не `0`.
...
Модуль `sudo` показує, чи є зараз облікові дані sudo в кеші.
...
Модуль `swift ` показує поточну встановлену версію [Swift](https://swift.org/).
Модуль показується, якщо виконується будь-яка з наступних умов:
...
Модуль `terraform` показує поточний вибраний [робочий простір Terraform](https://www.terraform.io/docs/language/state/workspaces.html) та його версію.
Він підтримує як Hashicorp Terraform, так і OpenTofu для виявлення версій.

> [!TIP] Порада
> Типово версія Terraform/OpenTofu не показується, оскільки це повільно для поточних версій Terraform, коли використовується багато втулків.
> Якщо ви все ще хочете увімкнути показ версії, [дивіться  приклад нижче](#with-terraform-version).

Типово, модуль показується, якщо виконується будь-яка з наступних умов:

- Поточна тека містить теку `.terraform`
...
Модуль `time` показує поточний місцевий час.
Значення конфігурації `format` використовується крейтом [`jiff`](https://crates.io/crates/jiff) для керування способом показу часу. Ознайомтеся [з документацією до бібліотеки jiff strftime](https://docs.rs/jiff/latest/jiff/fmt/strtime/index.html), щоб дізнатися, які параметри доступні.
...
| `time_format`     | дивіться нижче          | [Строка форматування jiff](https://docs.rs/jiff/latest/jiff/fmt/strtime/index.html), що використовується для форматування часу.                                                                                   |
| `style`           | `'bold yellow'`         | Стиль модуля time                                                                                                                                                                                                                 |
| `utc_time_offset` | `'local'`               | Встановлює зсув від UTC. Або назва часового поясу IANA, або діапазон від -24 &lt; x &lt; 24. Дозволяє часові пояси із сувом 30/45 хвилин. |
...
Якщо `use_12hr` є `true`, то стандартно `time_format` — `'%r'`. В іншому випадку стандартне значення — `'%T'`.
Налаштований вручну `time_format` має перевагу над параметром `use_12hr`.
...
Модуль `username` показує імʼя активного користувача.
Модуль показується, якщо виконується будь-яка з наступних умов:
...
> [!TIP] Порада
> SSH-зʼєднання виявляється шляхом перевірки змінних оточення `SSH_CONNECTION`, `SSH_CLIENT` та `SSH_TTY`. Якщо ваш хост SSH не налаштував ці змінні, одним зі способів розвʼязання проблеми є встановлення для однієї з них фіктивного значення.
...
Модуль `vagrant ` показує поточну встановлену версію [Vagrant](https://www.vagrantup.com/).
Типово, модуль показується, якщо виконується будь-яка з наступних умов:
...
Модуль `vlang ` показує поточну встановлену версію [V](https://vlang.io/).
Типово, модуль показується, якщо виконується будь-яка з наступних умов:
...
> Зверніть увагу, що модуль є стандартно увімкненим, але **не** є у списку стандартних модулів, оскільки це була б суттєва зміна.
> Крім того, точний формат модуля може змінитися в майбутньому, наприклад, для обробки вирівнювання праворуч.

Модуль `vcs` показує поточну активну систему контролю версій (VCS).
...
Модуль `vcsh` показує поточний репозиторій [VCSH](https://github.com/RichiH/vcsh).
...
Модуль `zig ` показує поточну встановлену версію [Zig](https://ziglang.org/).
Модуль показується, якщо виконується будь-яка з наступних умов:
...
> [!TIP] Порада
> Кілька власних модулів можна визначити за допомогою `.`.

> [!TIP] Порада
> Порядок показу власних модулів можна встановити індивідуально, включивши `${custom.foo}` у `format` верхнього рівня (оскільки він містить крапку, потрібно використовувати `${...}`).
> Типово, модуль `custom` покаже усі модулі custom, в тому порядку, в якому вони були визначені.

> [!TIP] Порада
> [Тікет #125](https://github.com/starship/starship/discussions/1252) містить приклади власних модулів.
> Якщо у вас є цікавий приклад ще не розкритий там, не соромтеся, поділитися ним!

> [!WARNING] Попередження
> Якщо ввімкнено параметр `unsafe_no_escape` або у версіях Starship до v1.20, вихідні дані команд виводяться у командний рядок без екранування.
>
...
> Такі послідовності зазвичай залежать від оболонки, наприклад, ви можете написати командний модуль, який створює послідовності для bash, наприклад, `\h`, але цей модуль не працюватиме в оболонці fish або zsh.
>
> Строки форматування також можуть містити послідовності команд, характерні для конкретної оболонки, наприклад: [Bash](https://www.gnu.org/software/bash/manual/html_node/Controlling-the-Prompt.html), [Zsh](https://zsh.sourceforge.io/Doc/Release/Prompt-Expansion.html).
...
| `os`                |                                 | Назва операційної системи, на якій буде показано модуль (unix, linux, macos, windows, … ) [Перегляньте можливі значення](https://doc.rust-lang.org/std/env/consts/constant.OS.html).                                                                                                                                                                             |
...
#### Власні команди shell {#custom-command-shell}
...
Якщо `shell` не вказано або міститься тільки один елемент і Starship виявить, що буде використано PowerShell, наступні аргументи будуть автоматично додані: `-Noprofile -Command -`.
Якщо `shell` не вказано або міститься лише один елемент і Starship виявить, що буде використано Cmd, аргумент `/C`  буде автоматично додано, а stdin буде встановлено у `false``.
Якщо параметр `shell`не вказано або він містить лише один елемент, і Starship виявляє, що буде використовуватися Nushell,
автоматично додадуться такі аргументи:`-c`та`stdin`будуть встановлені на`false\`.
...
> [!WARNING] Попередження
> Переконайтеся, що ваша власна конфігурація оболонки відповідним чином завершує роботу
>
...
> [Будь ласка, створітьтікет](https://github.com/starship/starship/issues/new/choose)
> з подробицями з терміналу і конфігурацією автора, якщо ви потрапили в такий сценарій.
