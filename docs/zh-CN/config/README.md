...
其中 session key 与您的终端实例相对应。 不过，也可以使用 `STARSHIP_CACHE` 环境变量来修改：
...
**Module**: A component in the prompt giving information based on contextual information from your OS. 例如，如果您当前的目录是 Node.js 项目，“nodejs” 组件会显示当前安装在您电脑上的 Node.js 的版本。

**Variable**: Smaller sub-components that contain information provided by the module. 例如，“nodejs”组件里的“version”字段包含了现有 Node.js 的版本。
...
你可以在当中添加文本、字段，甚至嵌套其他文本组。

In the second part, which is enclosed in a `()`, is a [style string](#style-strings). 用于指定第一部分括号中内容的样式。
...
Starship 中的大多数组件允许您为其设置显示样式。 This is done with an entry (usually called `style`) which is a string specifying the configuration. 以下的例子给出了一些样式字符串并描述了它们的效果。 For details on the full syntax, consult the [advanced config guide](../advanced-config/).
...
Many modules have `detect_extensions`, `detect_files`, and `detect_folders` variables. 这些变量接受用于匹配或不匹配的字符串。 反向匹配指用于指定不匹配的字符串，以字符 '!' 开头。 The presence of _any_ negative indicator in the directory
will result in the module not being matched.

对于文件扩展名，文件名中第一个点后的内容和最后一个点后的内容会被分别匹配一次。 For example, `foo.bar.tar.gz` will be matched
against `bar.tar.gz` and `gz` in the `detect_extensions` variable. 文件名以点开头的文件则被视为没有扩展名。
...
| `palettes`        | `{}`                          | Collection of color palettes that assign [colors](../advanced-config/#style-strings) to user-defined names. 注意调色板不能引用自定义的名称作为颜色。 |
...
The default `format` is used to define the format of the prompt, if empty or no `format` is provided. 默认配置如下：
...
modules you explicitly add to the format will not be duplicated. 例如：
...
> 若要启用此组件，请在配置文件中设置 `disable` 字段为 `false`。

### 该字段的内容显示于当前仓库的 hg 书签或活动分支名之前。

| `cmake` 组件显示当前安装的 [CMake](https://cmake.org/) 版本。 默认情况下，此组件将在满足以下任意条件时启用： | 默认值                                      | 描述                                                                                                    |
| ------------------------------------------------------------------------- | ---------------------------------------- | ----------------------------------------------------------------------------------------------------- |
...
| 此字段的内容用来表示分支名称被截断。 | 示例       | 描述                                   |
| ------------------ | -------- | ------------------------------------ |
| `version`          | `v1.0.0` | The version of `buf`                 |
| `符号`               |          | Mirrors the value of option `symbol` |
| `style`\*          |          | Mirrors the value of option `style`  |
...
| 当前目录是软件包的代码仓库时，将显示 `package` 组件，并显示软件包当前版本。 The module currently supports `npm`, `nimble`, `cargo`, `poetry`, `python`, `composer`, `gradle`, `julia`, `mix`, `helm`, `shards`, `galaxy`, `daml` and `dart` packages. | 示例                                     | 描述                                   |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------- | ------------------------------------ |
| name                                                                                                                                                                                                                                  | clang                                  | 编译器的名称                               |
...
默认情况下只会更改颜色。 If you also want to change its shape take a
...
| `username` 组件显示当前活跃的用户名。 此组件将在符合以下任意条件时显示： | 示例 | 描述                                                                                                                       |
| ------------------------------------------ | -- | ------------------------------------------------------------------------------------------------------------------------ |
...
| `java` 组件显示当前安装的 [Java](https://www.oracle.com/java/) 版本。 默认情况下，此组件将在满足以下任意条件时显示： | 示例        | 描述                                   |
| --------------------------------------------------------------------------------- | --------- | ------------------------------------ |
...
| `nodejs` 组件显示当前安装的 [Node.js](https://nodejs.org/) 版本。 默认情况下，此组件将在满足以下任意条件时显示： | 示例         | 描述                                   |
| --------------------------------------------------------------------------------------------- | ---------- | ------------------------------------ |
...
只需要在执行 `eval $(starship init $0)` 前简单地定义 `preexec_functions` 和 `precmd_functions` 两个列表，就可以照常运行了。
...
| `git_state` 组件会显示当前目录在哪个 git 仓库中，以及正在进行的操作，例如：_REBASING_，_BISECTING_ 等。 进度信息（例如 REBASING 3/10）如果存在则也会被显示。 | 示例           | 描述                                   |
| --------------------------------------------------------------------------------------------------------- | ------------ | ------------------------------------ |
...
| [!WARNING] Make sure your custom shell configuration exits gracefully | 示例                  | 描述                                   |
| ------------------------------------------------------------------------------------------------------------------------- | ------------------- | ------------------------------------ |
...
| `deno` 组件显示当前安装的 [Deno](https://deno.land/) 版本。 默认情况下，此组件将在满足以下任意条件时显示： | 示例       | 描述                                   |
| ----------------------------------------------------------------------- | -------- | ------------------------------------ |
...
| 如果这个 conda 环境是通过 `conda create -p [path]` 创建的，环境路径的目录深度应该被截断到此数量。 `0` 表示不用截断。 另请参阅 [`directory`](#directory) 组件。 | 示例       | 描述                                   |
| ---------------------------------------------------------------------------------------------------------------- | -------- | ------------------------------------ |
...
three parent folders. 如果您处于一个 git 仓库中，显示的路径则最多会截断到该仓库的根目录。
...
<details><summary>此组件有几个高级配置选项来控制当前目录路径的显示方式。</summary>
...
| 此字段的内容用来表示分支名称被截断。 You can use `''` for no symbol. | 示例                    | 描述                                  |
| ------------------------------------------------------------------ | --------------------- | ----------------------------------- |
...
| The `solidity` module shows the currently installed version of [Solidity](https://soliditylang.org/) The module will be shown if any of the following conditions are met: | 示例             | 描述                                   |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- | ------------------------------------ |
...
The `dotnet` module shows the relevant version of the [.NET Core SDK](https://dotnet.microsoft.com/) for the current directory. 如果当前目录已被绑定了一个版本的 SDK，则显示被帮定的版本。 否则此组件将显示最新安装的 SDK 版本。 否则此组件将显示最新安装的 SDK 版本。
...
在内部，此组件使用自己的版本检测机制。 Typically it is twice as fast
...
| `directory` 组件显示当前目录的路径，显示的路径会截断到三个父目录以内。 如果您处于一个 git 仓库中，显示的路径则最多会截断到该仓库的根目录。 | 示例               | 描述                                                                 |
| ------------------------------------------------------------------------------ | ---------------- | ------------------------------------------------------------------ |
...
| `battery` 组件显示电池充电情况和当前充电状态。 这个组件只会在当前电量低于 10% 时显示。 | 示例      | 描述                                   |
| --------------------------------------------------- | ------- | ------------------------------------ |
...
| 在内部，此组件使用自己的版本检测机制。 一般来说此组件是直接执行 `dotnet --version` 的两倍快，但当你的 .NET 项目使用了不常见的目录布局时此组件可能显示一个错误的版本。 如果相比于速度您更需要正确的版本号，您可以在组件设置中设置 `heuristic = false` 来禁用该机制。 | 示例                                                             | 描述                                         |
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- | ------------------------------------------ |
...
| 当前目录包含一个 `template.typ` 文件 | 示例        | 描述                                   |
| -------------------------- | --------- | ------------------------------------ |
...
| # 在 Windows 上# $HOME\.starship\config.toml[sudo]&#xA;allow_windows = true&#xA;disabled = false | 示例       | 描述                                   |
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ------------------------------------ |
...
| 选项                  | 默认值                                                                                                                         | 描述                                                                             |
| ------------------- | --------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| `符号`                | `' '`                                                                                                                      | The symbol used before displaying the version of Fortran.      |
| `format`            | `'via [$symbol($version )]($style)'`                                                                                        | 组件格式化模板。                                                                       |
| `version_format`    | `'${raw}'`                                                                                                                  | 版本格式 Available vars are `raw`, `major`, `minor`, & `patch` |
| `style`             | `'bold purple'`                                                                                                             | 此组件的样式。                                                                        |
...
| `bun` 组件显示当前安装的 [bun](https://bun.sh) JavaScript 运行时版本。 默认情况下，此组件将在满足以下任意条件时显示： | 示例       | 描述                                   |
| ------------------------------------------------------------------------------- | -------- | ------------------------------------ |
...
| active    | `示例`          | The active config name written in `~/.config/gcloud/active_config` |
| symbol    |               | Mirrors the value of option `symbol`                               |
| style\*   |               | Mirrors the value of option `style`                                |
...
_BISECTING_, etc. 进度信息（例如 REBASING 3/10）如果存在则也会被显示。
...
| `trim_at`         | `'.'`                                  | 当主机名过长被截断时，会截断成第一次匹配该字符串之前的主机名。 `'.'` will stop after the first dot. `'.'` will stop after the first dot. `''` will disable any truncation. |
...
仅当有后台任务运行时，此组件才会显示。
`jobs` 组件显示当前正在运行的任务数量。 仅当有后台任务运行时，此组件才会显示。 The module will show the number of jobs running if there are at least 2 jobs, or more than the `number_threshold` config value, if it exists.
...
The `rlang` module shows the currently installed version of [R](https://www.r-project.org/).
...
| `format`            | `'via [$symbol($version )]($style)'` | 组件格式化模板。                                                                       |
| `version_format`    | `'v${raw}'`                          | 版本格式 Available vars are `raw`, `major`, `minor`, & `patch` |
| `符号`                | `'🅼 '`                              | A format string representing the symbol of Maven.              |
| `detect_extensions` | `[]`                                 | 触发此组件的扩展名                                                                      |
...
| 选项                  | 默认值                                                                  | 描述                                                               |
| ------------------- | -------------------------------------------------------------------- | ---------------------------------------------------------------- |
| `符号`                | `'mise '`                                                            | The symbol used before displaying _mise_ health. |
| `style`             | `'bold purple'`                                                      | 此组件的样式。                                                          |
...
| `符号`                | `'🔥 '`                               | The symbol used before displaying the version of Mojo. |
...
| ---------- | --------------------------------- | ---------------------------------------------------------------------------------------------------- |
| `format`   | `'[$symbol \[$name\]]($style)'` | 组件格式化模板。                                                                                             |
| `符号`       | `'🛜 '`                           | The symbol used before the network namespace (defaults to empty). |
...
| `符号`                | `'Ø '`                               | The symbol used before displaying the version of Odin. |
...
| Parameters similar to `-NoProfile` in PowerShell are recommended for other shells as well to avoid extra loading time of a custom profile on every starship invocation. | 触发此组件的文件名                            | 描述                                                                             |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ | ------------------------------------------------------------------------------ |
| `format`                                                                                                                                                                                | `'via [$symbol($version )]($style)'` | Username 用户名                                                                   |
| `version_format`                                                                                                                                                                        | `'v${raw}'`                          | 版本格式 Available vars are `raw`, `major`, `minor`, & `patch` |
| `符号`                                                                                                                                                                                    | `'⨁ '`                               | A format string representing the symbol of Quarto                              |
| `style`                                                                                                                                                                                 | `'bold #75AADB'`                     | 时间                                                                             |
...
| 触发此组件的扩展名 | [!TIP] Multiple custom modules can be defined by using a `.`. | If you set a custom command, make sure that the default Shell used by starship will properly execute the command with a graceful exit (via the `shell` option). |
| --------- | --------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| **字段（Variable）**：组件中显示信息的部分。 例如，“nodejs”组件里的“version”字段包含了现有 Node.js 的版本。 | 触发此组件的文件名                            | 描述                                                                                                                                                                                                            |
| ----------------------------------------------------------------------------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `format`                                                                                  | `'via [$symbol($version )]($style)'` | 当前时间                                                                                                                                                                                                          |
| `version_format`                                                                          | `'v${raw}'`                          | 版本格式 Available vars are `raw`, `major`, `minor`, & `patch`                                                                                                                                |
| `符号`                                                                                      | `'📐'`                               | A format string representing the symbol of R.                                                                                                                                                 |
| `style`                                                                                   | `'blue bold'`                        | [!WARNING] If `unsafe_no_escape` is enabled or prior to starship v1.20 command output is printed unescaped to the prompt. |
...
| 触发此组件的扩展名 | 如果 `shell` 未设置或仅包含一个元素，并且 Starship 检测到正在使用 PowerShell，则会自动添加 `-NoProfile -Command -` 参数。 如果 `shell` 未设置或仅包含一个元素，并且 Starship 检测到正在使用 Cmd，则会自动添加 `/C` 参数，并且将 `stdin` 设置为 `false`。 如果 `shell` 未设置或仅包含一个元素，并且 Starship 检测到正在使用 Cmd，则会自动添加 `-C` 参数，并且将 `stdin` 设置为 `false`。 可以通过将参数显式传递给 shell 来避免这种行为，例如： | 非 root 用户使用的样式。                      |
| --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ |
...
| 以下是关于提示符的配置项。                   | 版本格式 可用的字段有 `raw`, `major`, `minor` 和 `patch` | 默认情况下只会更改颜色。 如果您也想更换字符，请查看 [此示例](#with-custom-error-shape)。 |
| ------------------------------- | --------------------------------------------- | ----------------------------------------------------------- |
...
### 版本格式 可用的字段有 `raw`, `major`, `minor` 和 `patch`
...
| 触发此组件的文件名           | 触发此组件的文件名                            | 描述                                                                             |
| ------------------- | ------------------------------------ | ------------------------------------------------------------------------------ |
| `format`            | `'via [$symbol($version )]($style)'` | 选项                                                                             |
| `version_format`    | `'v${raw}'`                          | 版本格式 Available vars are `raw`, `major`, `minor`, & `patch` |
...
| 触发此组件的文件名 | 版本格式 可用的字段有 `raw`, `major`, `minor` 和 `patch` | 禁用 `ruby` 组件。                        |
| --------- | --------------------------------------------- | ------------------------------------ |
...
| `style`             | `'bold red'`                         | 选项                                                                             |
| `disabled`          | `false`                              | 见下文解释                                                                          |

### 触发此组件的文件名

| 触发此组件的文件名 | 版本格式 可用的字段有 `raw`, `major`, `minor` 和 `patch` | 版本格式。 可用的字段有 `raw`, `major`, `minor` 和 `patch`              |
| --------- | --------------------------------------------- | ----------------------------------------------------------- |
...
| `format`               | `'[$indicator]($style) '` | 选项                                                                                                                                     |
| `style`                | `'white bold'`            | 选项                                                                                                                                     |
| `disabled`             | `true`                    | Disables the `shell` module.                                                                                           |

### 触发此组件的文件名

| 触发此组件的文件名 | 触发此组件的文件名 | 版本格式 可用的字段有 `raw`, `major`, `minor` 和 `patch`                              |
| --------- | --------- | -------------------------------------------------------------------------- |
...
| Format strings can also contain shell specific prompt sequences, e.g. [Bash](https://www.gnu.org/software/bash/manual/html_node/Controlling-the-Prompt.html), [Zsh](https://zsh.sourceforge.io/Doc/Release/Prompt-Expansion.html). | 触发此组件的文件夹                    | 描述                                                                            |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------- | ----------------------------------------------------------------------------- |
| `threshold`                                                                                                                                                                                                                                                                        | `2`                          | Display threshold.                                            |
| `format`                                                                                                                                                                                                                                                                           | `'[$symbol$shlvl]($style) '` | 选项                                                                            |
| `符号`                                                                                                                                                                                                                                                                               | `'↕️  '`                     | The symbol used to represent the `SHLVL`.                     |
| `repeat`                                                                                                                                                                                                                                                                           | `false`                      | Causes `symbol` to be repeated by the current `SHLVL` amount. |
| `repeat_offset`                                                                                                                                                                                                                                                                    | `0`                          | Decrements number of times `symbol` is repeated by the offset value           |
| `style`                                                                                                                                                                                                                                                                            | `'bold yellow'`              | 选项                                                                            |
| `disabled`                                                                                                                                                                                                                                                                         | `true`                       | Disables the `shlvl` module.                                  |

### 触发此组件的文件夹

| 触发此组件的文件夹 | 版本格式 可用的字段有 `raw`, `major`, `minor` 和 `patch` | 版本格式 可用的字段有 `raw`, `major`, `minor` 和 `patch` |
| --------- | --------------------------------------------- | --------------------------------------------- |
...
| 组件格式化模板。   | 触发此组件的文件夹                        | 描述                                                               |
| ---------- | -------------------------------- | ---------------------------------------------------------------- |
| `format`   | `'[$symbol\[$env\]]($style) '` | 选项                                                               |
| `符号`       | `''`                             | A format string displayed before the image name. |
| `style`    | `'bold dimmed blue'`             | 选项                                                               |
...
| ------------------- | ------------------------------------------------------------ | ------------------------------------------------------------------------------ |
| `format`            | `'via [$symbol($version )]($style)'`                         | 选项                                                                             |
| `version_format`    | `'v${major}.${minor}.${patch}'`                              | 版本格式 Available vars are `raw`, `major`, `minor`, & `patch` |
...
| `符号`                | `'🅢  '`                               | 触发此组件的文件夹                                                                                                                                                                                      |
| `style`             | `'bold blue'`                          | 选项                                                                                                                                                                                             |
| `format`            | `'via [$symbol$environment]($style) '` | 选项                                                                                                                                                                                             |
...
| workspace | `示例`                                          | The current Terraform workspace               |
| symbol    |                                               | Mirrors the value of option `symbol`          |
| style\*   |                                               | Mirrors the value of option `style`           |
...
| 组件格式化模板。          | 触发此组件的文件夹               | 版本格式 可用的字段有 `raw`, `major`, `minor` 和 `patch`                                                                                                                                                                                                            |
| ----------------- | ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `format`          | `'at [$time]($style) '` | The format string for the module.                                                                                                                                                                                                        |
| `use_12hr`        | `false`                 | 第二个部分包含在 `()`中，内容是一个[样式字符串](#style-strings)， 用于指定第一部分括号中内容的样式。                                                                                                                                                                                           |
| `time_format`     | 见下文解释                   | 选项                                                                                                                                                                                                                                                       |
| `style`           | `'bold yellow'`         | 当前目录包含 `.rb` 文件                                                                                                                                                                                                                                          |
| `utc_time_offset` | `'local'`               | 设置所用 UTC 偏移量。 Range from -24 &lt; x &lt; 24. Either an IANA time zone name or a range from -24 &lt; x &lt; 24. 允许使用浮点数来得到 30/45 分钟的时区偏移。 |
| `disabled`        | `true`                  | 描述                                                                                                                                                                                                                                                       |
...
| 组件格式化模板。            | 触发此组件的文件夹                            | 版本格式 可用的字段有 `raw`, `major`, `minor` 和 `patch`                                  |
| ------------------- | ------------------------------------ | ------------------------------------------------------------------------------ |
| `format`            | `'via [$symbol($version )]($style)'` | 描述                                                                             |
| `version_format`    | `'v${raw}'`                          | 版本格式 Available vars are `raw`, `major`, `minor`, & `patch` |
| `符号`                | `'t '`                               | A format string representing the symbol of Typst                               |
| `style`             | `'bold #0093A7'`                     | 默认值                                                                            |
...
| typst_version | `示例`                                          | The current Typst version                                            |
| symbol                             |                                               | Mirrors the value of option `symbol`                                 |
| style\*                            |                                               | Mirrors the value of option `style`                                  |

配置项

## 版本格式 Available vars are `raw`, `major`, `minor`, & `patch`.
...
| 触发此组件的文件夹         | 触发此组件的文件夹               | 版本格式 可用的字段有 `raw`, `major`, `minor` 和 `patch`                                                |
| ----------------- | ----------------------- | -------------------------------------------------------------------------------------------- |
| `style_root`      | `'bold red'`            | The style used when the user is root/admin.                                  |
| `style_user`      | `'bold yellow'`         | 描述                                                                                           |
| `detect_env_vars` | `[]`                    | Which environment variable(s) should trigger this module. |
| `format`          | `'[$user]($style) in '` | 描述                                                                                           |
| `show_always`     | `false`                 | 配置项                                                                                          |
| `disabled`        | `false`                 | 示例                                                                                           |
...
| 触发此组件的文件夹           | 触发此组件的文件夹                            | 禁用 `terraform` 组件。                                                             |
| ------------------- | ------------------------------------ | ------------------------------------------------------------------------------ |
| `format`            | `'via [$symbol($version )]($style)'` | 描述                                                                             |
| `version_format`    | `'v${raw}'`                          | 版本格式 Available vars are `raw`, `major`, `minor`, & `patch` |
...
| 触发此组件的文件夹 | 禁用 `username` 组件。 | 触发此组件的文件夹                            |
| --------- | ----------------- | ------------------------------------ |
...
| 触发此组件的文件夹 | 触发此组件的文件夹 | 禁用 `time` 组件。                        |
| --------- | --------- | ------------------------------------ |
...
| `style`    | `'bold yellow'`                  | 默认值                                                                    |
| `format`   | `'vcsh [$symbol$repo]($style) '` | 描述                                                                     |
...
| `shell`             |                                 | [见下文解释](#custom-command-shell)                                                                                                                                                                                                                                                                                                                                   |
