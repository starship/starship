...
| `worktrunk`              | `false`                                                                                                                      | Whether or not to trim a [Worktrunk](https://worktrunk.dev/) `.<sanitized-branch>` suffix from the git repo root directory, where `/` and `\\` in the branch are replaced with `-` (for example, `repo.feature-foo` on branch `feature/foo` becomes `repo`). |
