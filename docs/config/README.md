...
The `p4` module is shown when the current directory is inside a Perforce workspace and
shows the username, the current workspace (client) and last revision the user has
if the user is logged with the `p4` command line tool with `p4 login`.
...
[p4]
format = '[ #$changelist $user@$client](140 bold) '
```
