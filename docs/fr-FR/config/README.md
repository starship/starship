...
Par défaut, Starship enregistre les avertissements et les erreurs dans un fichier nommé `~/.cache/starship/session_${STARSHIP_SESSION_KEY}.log`, ou la clé de session correspond à une instance de votre terminal. Ceci peut cependant être modifié en utilisant la variable d'environnement `STARSHIP_CACHE`:
...
**Variable**: Plus petit sous-composant contenant des informations fournies par le module. Par exemple, la variable "version" dans le module "nodejs" contient la version actuelle de NodeJS.
...
| `symbol`          | `"🐳 "`                             | Le symbole utilisé avant d'afficher le context Docker.                                 |
| `style`           | `"blue bold"`                      | Le style du module.                                                                     |
| `only_with_files` | `false`                            | Affiché quand il y a un `docker-compose.yml` ou un `Dockerfile` dans le dossier actuel. |
...
| Variable  | Exemple        | Description                            |
| --------- | -------------- | -------------------------------------- |
| context   | `test_context` | Le contexte actuel de Dockr            |
| symbol    |                | Reflète la valeur de l'option `symbol` |
| style\* |                | Reflète la valeur de l'option `style`  |
