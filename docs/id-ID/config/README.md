...
**Module**: A component in the prompt giving information based on contextual information from your OS. Sebagai contoh, modul "nodejs" menampilkan versi Node.js yang tengah terpasang pada komputermu, jika direktorimu sedang berada pada proyek Node.js.

**Variable**: Smaller sub-components that contain information provided by the module. Sebagai contoh, variabel "version" dalam modul "nodejs" memuat versi terikini dari Node.js.
...
Format string merupakan format yang sebuah modul gunakan untuk menampilkan semua variabelnya.
Most modules have an entry called `format` that configures the display format of the module.
Kamu bisa menggunakan teks, variabel, dan grup teks di dalam sebuah format string.
...
Kamu bisa menambahkan teks, variabel, atau bahkan grup teks bercabang di dalamnya.
...
Sebagian besar modul starship memungkinkan kamu untuk mengkonfigurasi gaya tampilannya. This is done with an entry (usually called `style`) which is a string specifying the configuration. Berikut adalah beberapa contoh penataan pada string dan kegunaannya. For details on the full syntax, consult the [advanced config guide](../advanced-config/).
...
The default `format` is used to define the format of the prompt, if empty or no `format` is provided. Aturannya seperti yang ditunjukkan:
...
Modulnya hanya dapat terlihat ketika baterai perangkat di bawah 10%.
...
| `unknown_symbol`     | `'󰂑 '`                           | Simbol dimunculkan ketika keadaan baterai tidak dikenali. |
...
If no `display` is provided. Aturannya seperti yang ditunjukkan:
...
Secara bawaan karakter hanya dapat mengganti warna. If you also want to change its shape take a
...
The `cmake` module shows the currently installed version of [CMake](https://cmake.org/). Secara bawaan, modul akan aktif jika beberapa syarat berikut telah terpenuhi:
...
three parent folders. Direkrotimu juga akan disingkat ke root dari git repo di tempatmu berada saat ini.
...
<details><summary>Modul ini memilki beberapa opsi konfigurasi lanjutan yang mengontrol bagaimana direktori ditampilkan.</summary>
...
The `dotnet` module shows the relevant version of the [.NET Core SDK](https://dotnet.microsoft.com/) for the current directory. Apabila SDK telah disematkan pada direktori terkni, maka veri yang telah disematkan tersebutlah yang ditampilkan. Jika tidak, maka modul akan menampilkan versi SDK terkini.
...
Secara internal, modul ini menggunakan mekenasimenya sendiri untuk melakukan pendeteksian versi. Typically it is twice as fast
...
Modul hanya akan aktif jika beberapa syarat berikut telah terpenuhi:
...
| `format`    | `"with [$symbol$env_value]($style) "` | Format dari modul.                                                                    |
...
| `format`            | `'via [$symbol($version )]($style)'`                                                                                        | Format dari modul.                                                                           |
| `version_format`    | `'${raw}'`                                                                                                                  | Format dari versi. Available vars are `raw`, `major`, `minor`, & `patch` |
| `style`             | `'bold purple'`                                                                                                             | Gaya penataan untuk modul.                                                                   |
...
| `style`             | `'bold purple'`                           | Gaya penataan untuk modul.                                                                   |
| `format`            | `'on [$symbol$branch(:$topic)]($style) '` | Format dari modul.                                                                           |
...
| `style`      | `'bold yellow'`             | Gaya penataan untuk modul.                                    |
| `format`     | `'\([$state]($style)\) '` | Format dari modul.                                            |
...
| `style`             | `'bold purple'`                                                      | Gaya penataan untuk modul.                       |
| `format`            | `'on [$symbol$health]($style) '`                                     | Format dari modul.                               |
| `detect_extensions` | `[]`                                                                 | Ekstensi mana yang sebaiknya memicu modul ini.   |
...
| Opsi                | Bawaan                                | Deskripsi                                                              |
| ------------------- | ------------------------------------- | ---------------------------------------------------------------------- |
| `format`            | `'with [$symbol($version )]($style)'` | Format dari modul.                                     |
...
| `style`             | `'bold purple'`                   | Gaya penataan untuk modul.                                                           |
| `format`            | `'on [$symbol$channel]($style) '` | Format dari modul.                                                                   |
...
| Opsi                       | Bawaan                                                    | Deskripsi                                                                                                                     |
| -------------------------- | --------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `format`                   | `'via [$symbol($version )(\($environment\) )]($style)'` | Format dari modul.                                                                                            |
| `version_format`           | `'v${raw}'`                                               | Format dari versi. Available vars are `raw`, `major`, `minor`, & `patch`. |
| `symbol`                   | `'🧚 '`                                                   | Simbol yang digunakan sebelum nama environment.                                                               |
| `style`                    | `'yellow bold'`                                           | Gaya penataan untuk modul.                                                                                    |
...
| `detect_extensions` | `['p6', 'pm6', 'pod6', 'raku', 'rakumod']`       | Menyalin nilai dari opsi `style`                                                                             |
...
| `detect_extensions` | `['rs']`                             | Menyalin nilai dari opsi `symbol`                                                                            |
| `detect_files`      | `['Cargo.toml']`                     | Menyalin nilai dari opsi `style`                                                                             |
| `detect_folders`    | `[]`                                 | Folder mana yang sebaiknya memicul modul ini.                                                |
| `style`             | `'bold red'`                         | Gaya penataan untuk modul.                                                                   |
...
| `format`            | `'via [${symbol}(${version} )]($style)'` | Format dari modul.                                                                           |
| `version_format`    | `'v${raw}'`                              | Format dari versi. Available vars are `raw`, `major`, `minor`, & `patch` |
| `detect_extensions` | `['sbt', 'scala']`                       | Menyalin nilai dari opsi `style`                                                                             |
...
| `format`        | `'[$symbol$shlvl]($style) '` | Menyalin nilai dari opsi `symbol`                                             |
...
| `style`    | `'bold dimmed blue'`             | Menyalin nilai dari opsi `style`                                 |
...
| ------------------- | ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------ |
| `format`            | `'via [$symbol($version )]($style)'`                         | Format dari modul.                                                                           |
| `version_format`    | `'v${major}.${minor}.${patch}'`                              | Format dari versi. Available vars are `raw`, `major`, `minor`, & `patch` |
...
| `symbol`            | `'🅢  '`                               | Menyalin nilai dari opsi `symbol`                                                                                                                                                              |
| `style`             | `'bold blue'`                          | Menyalin nilai dari opsi `style`                                                                                                                                                               |
| `format`            | `'via [$symbol$environment]($style) '` | Format dari modul.                                                                                                                                                             |
...
| `detect_extensions` | `['tf', 'tfplan', 'tfstate']`                           | Menyalin nilai dari opsi `style`                                                                             |
| `detect_files`      | `[]`                                                    | filenames mana yang sebaiknya memicu modul ini.                                              |
