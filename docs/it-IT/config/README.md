...
Le stringhe di formato sono il formato con cui un modulo stampa tutte le sue variabili.
La maggior parte dei moduli ha una voce chiamata `formato` che configura il formato di visualizzazione del modulo.
È possibile utilizzare testi, variabili e gruppi di testo in una stringa di formato.

#### Variable

Una variabile contiene un simbolo `$` seguito dal nome della variabile.
...
La prima parte, che è racchiusa tra `[]`, è una [format string](#format-strings).
È possibile aggiungere testi, variabili o anche gruppi annidati di testo.
...
| `unknown_symbol`     | `'󰂑 '`                           | Il simbolo mostrato quando lo stato della batteria è sconosciuto. |
