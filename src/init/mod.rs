...
// using whatever mechanism is available in the host shell--this two-phase
// solution has been developed as a compatibility measure with `eval $(starship
// init X)`
...
source /dev/stdin <<<"$("{}" init bash --print-full-init)"
        ...
        Some("fish") => {
            ...
            let script = format!("source (\"{}\" init fish --print-full-init | psub)", starship);
            Some(script)
        }
        None => {
            println!(
        ...
        Some(shell_basename) => {
            println!(
                "printf \"\\n{0} is not yet supported by starship.\\nFor the time being, we support bash, zsh, and \
...
//
// Each init script will be passed as-is. Global notes for init scripts are in
// this comment, with additional per-script comments in the strings themselves.
//
// JOBS: The argument to `--jobs` is quoted because MacOS's `wc` leaves
// whitespace in the output. We pass it to starship and do the whitespace
// removal in Rust, to avoid the cost of an additional shell fork every shell
// draw.
//
// Note that the init scripts are not in their final form--they are processed by
// `starship init` prior to emitting the final form. In this processing, some
// tokens are replaced, e.g. `::STARSHIP::` is replaced by the full path to the
// starship binary.
