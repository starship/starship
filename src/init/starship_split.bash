...
starship_precmd() {
    ...
    if [[ $STARSHIP_START_TIME ]]; then
        ...
        PS1="$(::STARSHIP:: prompt --character-only --status=$STATUS --jobs="$(jobs -p | wc -l)" --cmd-duration=$STARSHIP_DURATION)"
        PS2="$(::STARSHIP:: prompt --status=$STATUS --jobs="$(jobs -p | wc -l)" --cmd-duration=$STARSHIP_DURATION)"
        unset STARSHIP_START_TIME
    else
        PS1="$(::STARSHIP:: prompt --character-only --status=$STATUS --jobs="$(jobs -p | wc -l)")"
        PS2="$(::STARSHIP:: prompt --status=$STATUS --jobs="$(jobs -p | wc -l)")"
    fi
