...
fn parse_toml_value(value: &str) -> Value {
    toml::de::ValueDeserializer::parse(value)
        .ok()
        .and_then(|deserializer| Value::deserialize(deserializer).ok())
...
mod tests {
    ...
    fn test_env_config_wrong_type() {
        ...
        assert!(
            cfg.load_key_from_env("status__disabled__not_a_table", "true")
                .is_err()
        );
    ...
    fn test_env_config_simple() {
        ...
        assert!(
            !cfg.config.as_ref().unwrap()["status"]["disabled"]
                .as_bool()
                .unwrap()
        );
    ...
    fn test_update_config_deep() {
        ...
        assert!(
            cfg.config.unwrap()["a"]["b"]["c"]["d"]["e"]["f"]["g"]["h"]
                .as_bool()
                .unwrap()
        )
    }
