...
impl<'a> Default for DirectoryConfig<'a> {
    fn default() -> Self {
        DirectoryConfig {
            ...
            format: "[$dirs]($style)[$cwd]($cwd_style)[$read_only]($read_only_style) ",
            repo_root_format: "[$before_root_path]($style)[$repo_root]($repo_root_style)[$dirs]($style)[$cwd]($cwd_style)[$read_only]($read_only_style) ",
            style: "cyan bold",
            cwd_style: None,
