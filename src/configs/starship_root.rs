...
pub struct StarshipRootConfig {
    ...
    pub right_continuation_prompt: String,
...
impl<'a> Default for StarshipRootConfig {
    fn default() -> Self {
        StarshipRootConfig {
            ...
            right_continuation_prompt: " [∙](bright-black)".to_string(),
            scan_timeout: 30,
            command_timeout: 500,
            add_newline: true,
        }
    ...
    fn load_config(&mut self, config: &'a toml::Value) {
        if let toml::Value::Table(config) = config {
            config.iter().for_each(|(k, v)| match k.as_str() {
                ...
                "right_continuation_prompt" => self.right_continuation_prompt.load_config(v),
