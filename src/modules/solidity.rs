...
fn get_solcjs_version(context: &Context) -> Option<String>{
    let command=context.exec_cmd("solcjs", &["--version"])?;
    let solcjs_version_string =get_command_string_output(command);
    parse_solcjs_version(&solcjs_version_string)
}
...
#[cfg(test)]
mod tests {
    use super::parse_solcjs_version;
    ...
    fn folder_with_solidity_file() -> io::Result<()> {
        let dir = tempfile::tempdir()?;
        File::create(dir.path().join("test.sol"))?.sync_all()?;
        let actual = ModuleRenderer::new("solidity").path(dir.path()).collect();
