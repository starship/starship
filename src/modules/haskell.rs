    ...
    let parsed = StringFormatter::new(config.format).and_then(|formatter| {
        formatter
            ...
            .map(|variable| match variable {
                "compiler" => haskell_result.compiler.clone().map(Ok),
                "ghc_version" => haskell_result.compiler.clone().map(Ok), // TODO: The `ghc_version` field should be removed in favour of `compiler`.
...
fn get_version(haskell_result: &HaskellResult) -> Option<String> {
    haskell_result
        .snapshot
        .clone()
        .or(haskell_result.compiler.clone())
}
...
fn stack_result<'a>(context: &'a Context, config: &HaskellConfig) -> Option<HaskellResult> {
    ...
    maybe_project_root.map(|project_root| {
        ...
        HaskellResult {
            compiler: get_stack_compiler(context),
            snapshot: get_stack_snapshot(context, project_root),
        }
    })
}

fn get_stack_compiler(context: &Context) -> Option<String> {
    Some(
        context
            .exec_cmd(
                "stack",
...
fn cabal_result<'a>(context: &'a Context, config: &HaskellConfig) -> Option<HaskellResult> {
    ...
    maybe_project_root.map(|project_root| {
        ...
        HaskellResult {
            compiler: get_cabal_compiler(context),
            snapshot: None,
        }
    })
}

fn get_cabal_compiler(context: &Context) -> Option<String> {
...
fn plain_haskell_result<'a>(context: &'a Context, config: &HaskellConfig) -> Option<HaskellResult> {
    ...
    Some(HaskellResult {
        compiler: get_plain_compiler(context, config),
        snapshot: None,
    })
}

fn get_plain_compiler(context: &Context, config: &HaskellConfig) -> Option<String> {
    Some(
        context
...
mod tests {
    ...
    fn non_default_compiler() -> io::Result<()> {
        ...
        for hs_file in &should_trigger {
            ...
            let expected = Some(format!("via {}", Color::Purple.bold().paint("λ 0.13.3.0 ")));
            assert_eq!(expected, actual);
