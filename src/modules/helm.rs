...
pub fn module<'a>(context: &'a Context) -> Option<Module<'a>> {
    ...
    let helm_version = utils::exec_cmd("helm", &["version", "--short", "--client"])?.stdout;
    ...
    let parsed = StringFormatter::new(config.format).and_then(|formatter| {
        formatter
            ...
            .map(|variable| match variable {
                "version" => parse_helm_version(&helm_version).map(Ok),
                _ => None,
            })
            .parse(None)
    });
...
fn parse_helm_version(helm_version: &str) -> Option<String> {
...
mod tests {
    ...
    fn test_parse_helm_version() {
        ...
        assert_eq!(parse_helm_version(helm_2), Some("v2.16.9".to_string()));
        assert_eq!(parse_helm_version(helm_3), Some("v3.1.1".to_string()));
    }
}
