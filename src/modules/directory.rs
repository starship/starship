...
fn substitute_path(
    ...
    for substitution in substitutions {
        if substitution.regex.unwrap_or(false) {
            if let Ok(re) = Regex::new(substitution.from.as_str()) {
                substituted_dir = re
...
mod tests {
    ...
    fn substitute_prefix_and_middle() {
        ...
        let substitutions = First(vec![
            SubstitutionConfig::new("/absolute/path".to_string(), ""),
            SubstitutionConfig::new("/bar/".to_string(), "/"),
        ]);
