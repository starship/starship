...
";
    ...
    #[test]
    fn test_dynamic_parse_variable_extraction() {
        let str_file = r#"
            [elixir]
        ...
        let value: toml::Value = toml::from_str(str_file).unwrap();
        let config = ElixirConfig::try_load(Some(&value));

        assert_eq!(
            *config.version_extract_map.get("version").unwrap(),
            "Elixir\\s([\\d\\.\\-\\w]+)"
        );
        assert_eq!(
            *config.version_extract_map.get("otp_version").unwrap(),
            "Erlang/OTP\\s([\\d\\.\\-\\w]+)"
        );
    }
}
