...
impl<'a> Module<'a> {
    ...
    /// Like `ansi_strings`, but expands `$fill` segments to fit `width` columns.
    /// `shell` is used to discount the escapes it requires when measuring text.
    ...
    I: Iterator<Item = &'a Segment>,
{
    ...
    for segment in segments {
        if let Segment::Fill(fs) = segment {
            ...
            used += segment.width_graphemes(shell);
